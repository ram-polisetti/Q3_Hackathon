...
def main():
    ...
    try:
        # Get the simulator backend to check its qubit limit
        simulator = Aer.get_backend('aer_simulator')
        max_qubits = simulator.configuration().n_qubits

        # Calculate the maximum number of bits possible
        max_bits = (max_qubits - 1) // 4

        # Explain the qubit limits to the user before asking for input
        print("\n--- Quantum Simulator Qubit Limits ---")
        print(f"The current simulator can handle a maximum of {max_qubits} qubits.")
        print("This program uses 4 registers for each bit, plus one ancilla qubit.")
        print(f"The maximum number of bits you can use is {max_bits}.")

        # --- Configure your inputs here via user input ---
        num_bits = int(input(f"Enter the number of bits for the adder (1 to {max_bits}): "))

        # Calculate the required qubits and check against the limit
        required_qubits = 4 * num_bits + 1
        if required_qubits > max_qubits:
            print(f"Error: The number of bits ({num_bits}) requires {required_qubits} qubits, which exceeds the simulator's limit of {max_qubits}.")
            return

        max_decimal = 2**num_bits - 1
        a_val = int(input(f"Enter the first decimal value (0 to {max_decimal}): "))
        b_val = int(input(f"Enter the second decimal value (0 to {max_decimal}): "))

        if a_val < 0 or b_val < 0 or a_val > max_decimal or b_val > max_decimal:
            print(f"Error: Input values must be between 0 and {max_decimal} for a {num_bits}-bit adder.")
            return
